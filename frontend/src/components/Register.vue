...
// ✅ ปรับการดึง API_URL ให้รองรับ Proxy
...
const handleRegister = async () => {
  ...
  try {
    ...
    // 🚩 2. เช็คว่า Backend ส่ง Token กลับมาให้เลยไหม? (Auto Login)
    if (response.data.token && response.data.user) {
        // ถ้ามี Token -> บันทึกลง LocalStorage เลย (ถือว่า Login สำเร็จ)
        ...
        // ถ้า Backend ไม่ส่ง Token -> ต้องแจ้งให้ไป Login เอง (แบบเดิม)
        // (เผื่อ Backend ยังไม่ได้แก้)
        Swal.fire({
            icon: "success",
            title: "สมัครสมาชิกสำเร็จ!",
